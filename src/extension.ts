...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('Congratulations, your extension "arpcore-snippets" is now active!');

	// The command has been defined in the package.json file
	// Now provide the implementation of the command with registerCommand
	// The commandId parameter must match the command field in package.json
	let disposable = vscode.commands.registerCommand('arpcore-snippets.helloWorld', () => {
		// The code you place here will be executed every time your command is executed
		// Display a message box to the user
		vscode.window.showInformationMessage('Hello World from arpcore-snippets!');
	});

	let createPlugin = vscode.commands.registerCommand('arpcore-snippets.CreatePlugin', (uri: vscode.Uri) => {
		// Get the name of the plugin.
		let pluginNameInputBox = vscode.window.createInputBox();
		pluginNameInputBox.placeholder = "ARPCore Plugin Name";
		...
		vscode.window.showInformationMessage('Created ARPCore Plugin');
	});
